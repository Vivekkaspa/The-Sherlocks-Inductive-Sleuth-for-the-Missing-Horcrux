...
class SubgraphFewshotDataset(Dataset):
    def __init__(self, root, add_traspose_rels=False, shot = 1, n_query = 3, hop = 2, dataset='', mode='dev',kind = "union_prune_plus", preprocess = False, preprocess_50negs = False, skip= False, rev = False, use_fix2 = False, num_rank_negs = 50, inductive = False, orig_test = False):
        self.root = root
...
#         self.ssp_graph = ssp_graph
        ...
        if preprocess:
            ...
            self.main_env = lmdb.open(db_path, readonly=False, max_dbs=4, lock=False)
        ...
        if preprocess_50negs:     
            ...
            self.main_env = lmdb.open(db_path_50negs, readonly=False, max_dbs=3, lock=False)
        ...
        if (not preprocess) and (not preprocess_50negs) and (not skip):
            try:
    ...
    def _preprocess_50negs(self, num_rank_negs):
        ...
        all_neg_edge_index, all_neg_x, all_neg_x_id, all_neg_edge_attr = [], [], [], []
        all_neg_n_size, all_neg_e_size = [], []
        task_offsets_neg = []
        ...
        for index in tqdm(range(self.num_all_negs)):
            ...
            neg_edge_index, neg_x, neg_x_id, neg_edge_attr = [], [], [], []
            neg_n_size, neg_e_size = [], []
            ...
            with self.main_env.begin(db=self.db_50negs) as txn:
                for idx, i in enumerate(curr_tasks_neg_idx):
                    ...
                    if data is None:
                        print(f"Warning: Key {str_id} not found in database. Skipping.")
                        continue
                    
                    nodes_neg, r_label_neg, g_label_neg, n_labels_neg = deserialize(data).values()
                    d = self._prepare_subgraphs(nodes_neg, r_label_neg, n_labels_neg)
                    neg_edge_index.append(d.edge_index)
                    neg_x.append(d.x)
                    neg_x_id.append(d.x_id)
        ...
        if all_neg_edge_index:
            ...
            print("No negative examples were found for preprocessing.")
...
if __name__ == "__main__":
    #SubgraphFewshotDataset(".", shot = 3, dataset="NELL", mode="pretrain", kind="union_prune_plus", hop=2, preprocess = True, preprocess_50negs = True)
    SubgraphFewshotDataset(".", shot = 3, dataset="ConceptNet", mode="pretrain", kind="union_prune_plus", hop=1, preprocess = True, preprocess_50negs = True)
